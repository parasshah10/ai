<script lang="ts">
	...
	import { copyToClipboard, createMessagesList, removeAllDetails } from '$lib/utils';
	...
	const formatMessageContent = (message) => {
		...
		// If the message has files with text content, append them
		if (message.files) {
			const fileContents = message.files
				.filter((file) => file.file?.data?.content || file.data?.content)
				.map((file) => {
					const name = file.file?.filename || file.name || 'Untitled File';
					const text = file.file?.data?.content || file.data?.content;
					return `[File: ${name}]\n\`\`\`\n${text}\n\`\`\``;
				})
				.join('\n\n');
	...
	const getChatAsText = () => {
		...
		if (latestUserMessage) {
			chatText += formatMessageContent(latestUserMessage);
		}
